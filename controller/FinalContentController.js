...
const index = (req, res) => {
  ...
  if (items_per_pages && current_page_no) {
    ...
    var data =
      data +
      ` ORDER BY id DESC LIMIT ${items_per_pages} OFFSET ${current_page_starting_no} `; //offset - skip     fetch - take
  }
  // activity()
    ...
    if (err) {
      ...
      Database.getDb().query(
        ...
        function (error, countresult) {
          var result_data = {
            status: "success",
            total_count: countresult.length,
            data: result,
          };
...
const home = function (req, res) {
  ...
  Database.getDb().query(data, function (error, result) {
    try {
      ...
      res
        .status(500)
        .json({ status: "edit finalcontroller", line: 327, error: error });
    }
  });
};
// ============================create=============================
...
const create = async (req, res) => {
  ...
  var element = {
    ...
    web_html: req.body.web_html,
    web_css: req.body.web_css,
    mvna_html: req.body.mvna_html,
    mvna_css: req.body.mvna_css,
    mvna_style: req.body.mvna_style,
    mvna_components: req.body.mvna_components,
    mvna_assets: req.body.mvna_assets,
    slug: req.body.slug,
    created_by: 1,
    status: 1,
    updated_by: 1,
  ...
  if (id && id != "") {
    var sql = `UPDATE ${table_name} SET menu_id = '${
      element.menu_id
    }',web_html = ${JSON.stringify(
      JSON.stringify(element.web_html)
    )},web_css=${JSON.stringify(
      JSON.stringify(element.web_css)
    )},mvna_html = ${JSON.stringify(
      JSON.stringify(element.mvna_html)
    )},mvna_css=${JSON.stringify(
      JSON.stringify(element.mvna_css)
    )},mvna_styles=${JSON.stringify(
      JSON.stringify(element.mvna_style)
    )},mvna_components=${JSON.stringify(
      JSON.stringify(element.mvna_components)
    )},mvna_assets=${JSON.stringify(
      JSON.stringify(element.mvna_assets)
    )},slug=${JSON.stringify(JSON.stringify(element.slug))},updated_by = ${
      element.updated_by
    } WHERE id = ${id}`;
    var message = "Update";
  } else {
    var sql = `INSERT INTO ${table_name} (menu_id,web_html,web_css,mvna_html,mvna_css,mvna_styles,mvna_components,mvna_assets,slug,status,created_by) VALUES (${
      element.menu_id
    },${JSON.stringify(JSON.stringify(element.web_html))},${JSON.stringify(
      JSON.stringify(element.web_css)
    )},${JSON.stringify(JSON.stringify(element.mvna_html))},${JSON.stringify(
      JSON.stringify(element.mvna_css)
    )},${JSON.stringify(JSON.stringify(element.mvna_style))},${JSON.stringify(
      JSON.stringify(element.mvna_components)
    )},${JSON.stringify(JSON.stringify(element.mvna_assets))},${JSON.stringify(
      JSON.stringify(element.slug)
    )},${element.status},${element.created_by})`;
    var message = "Created";
  }
...
const customcreate = (req, res) => {
  ...
  if (mvna_html) {
    ...
    fs.appendFile(
      "./database/page/" + menu_id + "/mvna_html.json",
      `${JSON.stringify(element.mvna_html)}`,
      function (file_err) {
        if (file_err) {
    ...
    fs.appendFile(
      "./database/page/" + menu_id + "/mvna_css.json",
      `${JSON.stringify(element.mvna_css)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    fs.appendFile(
      "./database/page/" + menu_id + "/mvna_style.json",
      `${JSON.stringify(element.mvna_style)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    var sql = `UPDATE ${table_name} SET mvna_html="${
      "database/page/" + menu_id + "/mvna_html.json"
    }",mvna_css="${
      "database/page/" + menu_id + "/mvna_css.json"
    }",mvna_styles="${
    ...
    fs.writeFile(
      "./database/page/" + menu_id + "/mvna_components.json",
      `${JSON.stringify(element.mvna_components)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    var sql = `UPDATE ${table_name} SET mvna_components="${
      "database/page/" + menu_id + "/mvna_components.json"
    }" WHERE id = ${id}`;
    var message = "Update";
  } else if (mvna_assets) {
    fs.writeFile(
      "./database/page/" + menu_id + "/mvna_assets.json",
      `${JSON.stringify(element.mvna_assets)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    var sql = `UPDATE ${table_name} SET mvna_assets="${
      "database/page/" + menu_id + "/mvna_assets.json"
    }" WHERE id = ${id}`;
    var message = "Update";
  } else {
    ...
    fs.writeFile(
      "./database/page/" + menu_id + "/web_html.json",
      `${JSON.stringify(element.web_html)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    fs.writeFile(
      "./database/page/" + menu_id + "/web_css.json",
      `${JSON.stringify(element.web_css)}`,
      function (file_err) {
        if (file_err) {
          res.status(500).send(file_err);
    ...
    if (id) {
      var sql = `UPDATE ${table_name} SET menu_id=${
        element.menu_id
      },web_html="${"database/page/" + menu_id + "/web_html.json"}",web_css="${
        "database/page/" + menu_id + "/web_css.json"
      }",mvna_html="${
        "database/page/" + menu_id + "/mvna_html.json"
      }",mvna_css="${
        ...
        "database/page/" + menu_id + "/mvna_style.json"
      }",slug="${JSON.stringify(element.slug)}",status=${
        element.status
      },created_by=${element.created_by} WHERE id = ${id}`;
      var message = "Created";
    } else {
      var sql = `INSERT INTO ${table_name} (menu_id,web_html,web_css,mvna_html,mvna_css,mvna_styles,slug,status,created_by) VALUES (${
        element.menu_id
      },"${"database/page/" + menu_id + "/web_html.json"}","${
        "database/page/" + menu_id + "/web_css.json"
      }","${"database/page/" + menu_id + "/mvna_html.json"}","${
        "database/page/" + menu_id + "/mvna_css.json"
      }","${"database/page/" + menu_id + "/mvna_style.json"}",${JSON.stringify(
        JSON.stringify(element.slug)
      )},${element.status},${element.created_by})`;
      var message = "Created";
    }
...
const edit = (req, res) => {
  Database.getDb().query(
    `SELECT * FROM ${table_name} WHERE menu_id = ${req.params.id}`,
    function (error, result) {
      ...
      if (error) {
        ...
        try {
          ...
          res
            .status(500)
            .json({ status: "edit finalcontroler", line: 327, error: error });
        }
...
const get_pdf = (req, res, next) => {
var fs = require('fs');
var pdf = require('html-pdf');
var html = `<h2>hello world</h2> <script>  
...
pdf.create(html).toStream(function(err, stream){
  stream.pipe(fs.createReadStream(path.join(__dirname,'../docs/businesscard.pdf')));
});
/*
pdf.create(html, options).toFile( path.join(__dirname,'../docs/businesscard.pdf'), function(err, res) {
...
const get_pdf2 = (req, res, next) => {
  Database.getDb().query(
    `SELECT * FROM ${table_name} WHERE menu_id in (${req.params.id}) `,
    function (error, result) {
      //console.log(result);
      if (error) {
        res.status(500).send(error);
        ...
        try {
          if (result.length > 0) {
            ...
            @media all {
              ...
              <body>${JSON.parse(item.web_html)}</body>
...
const statusChange = (req, res) => {
  ...
  Database.getDb().query(
    `UPDATE ${table_name} SET status = ${req.params.status} WHERE id = ${req.params.id}`,
    function (error, result) {
      var result_data = {
        status: "success",
        message: "Page " + message + " Successfully",
      };
      res.status(200).json(result_data);
    }
  );
